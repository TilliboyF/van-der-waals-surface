...
class Intersection_Plane(Plane):
    ...
    def _calculate_plane(self, c1: list[float], c2: list[float], r1: float, r2: float) -> None:
    ...
    def calculate_intersection(self, v1: 'Vertex', v2: 'Vertex') -> float:
        ...
        t = (self.delta - self.alpha * v1.coords[0] - self.beta * v1.coords[1] - self.gamma * v1.coords[2])
        b = self.alpha * v[0] + self.beta * v[1] + self.gamma * v[2]

        if b == 0:
            if self.alpha * v1.coords[0] + self.beta * v1.coords[1] + self.gamma * v1.coords[2] - self.delta == 0:
                return v1.coords
            else:
                return 100
